...
class GuideStoreRequest extends FormRequest
{
    ...
    {
        return [
            'title' => ['required', 'string', 'between:3,100', Rule::unique('guides')],
            'icon' => ['nullable', 'string', 'between:3,30'],
            'program_link' => ['required', 'url', 'string', 'between:3,255', Rule::unique('guides')],
            'doc_link' => ['required', 'regex:/http:\/\/onlyoffice\.orlan\.in\/.*/', 'string', 'between:3,255', Rule::unique('guides')],
            'file' => ['required', File::types(['html'])->max(100 * 1024)],
